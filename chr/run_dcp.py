...
import torch
import random
from chr.chr.others import DCP
import numpy as np
import pandas as pd
...
def get_dcp(args):
    if os.path.exists("saved_results/{}_{}/dcp.pkl".format(args.dataset_name, args.seed)):
        with open("saved_results/{}_{}/dcp.pkl".format(args.dataset_name, args.seed), "rb") as f:
        ...
        method = DCP(bbox, ymin=np.min(y_train), ymax=np.max(y_train))


        method.calibrate(X_val, y_val, alpha)
        # Compute prediction on test data
        pred = method.predict(X_val)
        ...
        with open("saved_results/{}_{}/dcp.pkl".format(args.dataset_name, args.seed), "wb") as f:
            pickle.dump((coverages, lengths), f)
        with open("saved_results/{}_{}/dcp_preds.pkl".format(args.dataset_name, args.seed), "wb") as f:
            pickle.dump((pred), f)
